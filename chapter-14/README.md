...
chapter-19 で nginx をソースビルドした後も、同じ `tail -f` コマンドで動作確認ができるようになります。
...
chapter-19 で nginx をソースからビルドした後は、ログの保存先が変わります。
...
同じ `tail -f` コマンドで、chapter-19 以降もアクセスログをリアルタイムに確認できます。
