...
Java で書いてきたプログラムと、シェルスクリプトの構文を並べると違いが見えやすい。

| 構文 | シェルスクリプト | Java |
|:---|:---|:---|
| 変数宣言 | `NAME="nginx"` | `String name = "nginx";` |
| 変数参照 | `echo $NAME` | `System.out.println(name);` |
| 条件分岐 | `if [ $? -eq 0 ]; then ... fi` | `if (result == 0) { ... }` |
| ループ | `for f in *.log; do ... done` | `for (String f : files) { ... }` |
| 関数定義 | `function check() { ... }` | `void check() { ... }` |
| 戻り値 | `return 0`（終了コード：0 = 成功） | `return 0;`（任意の値） |
| コメント | `# コメント` | `// コメント` |
| 文末 | 改行（`;` 不要） | `;`（必須） |

シェルスクリプトはインタープリタが 1 行ずつ実行するため、型宣言や `;` が不要だ。
一方で空白の有無（`VAR=value` vs `VAR = value`）が意味を変えるため、Java の感覚では書けない点に注意。
