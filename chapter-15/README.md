...
Codespaces では SysVinit 互換の `service` コマンドで実習し、systemd の概念は chapter-20 の nginx.service 自作に向けて説明で理解する。
...
### 15-8. systemd ユニットファイルの構造を知る（説明 — chapter-20 の前準備）
...
chapter-20 では nginx をソースからビルドした後、このユニットファイルを参考に
