...
> **「パッケージ」とは?**
> ソフトウェア本体（**バイナリ**: コンパイル済みの実行ファイル。Windows の .exe に相当）・設定ファイルのひな型・マニュアル・依存関係の情報をひとまとめにしたファイルです。
...
| 概念 | 説明 | 例 |
|:---|:---|:---|
| **パッケージ** | ソフトウェアの梱包箱（`.deb` ファイル） | `tree_2.2.1-1_amd64.deb` |
...
> **`sudo` とは?**
> `sudo`（superuser do）はコマンドの先頭に付けることで、一時的に root（管理者）権限でそのコマンドを実行します。
> パッケージのインストールやシステム設定の変更には root 権限が必要なため、`apt` コマンドは `sudo` を付けて実行します。
> `sudo` の仕組みや設定方法は第9章（ユーザー管理）で詳しく学びます。
...
Debian trixie（この Codespaces 環境）では deb822 形式（Debian が導入した新しいリポジトリ設定形式。キーと値を複数行で記述するスタイル）の設定ファイルを使います。
...
`apt search` でパッケージが見つからなかったとき、または「どのバージョンが使えるのか」を確認したいとき、Suites と Components の知識が役に立ちます。
...
Package files:
 ...
 500 http://deb.debian.org/debian trixie/main amd64 Packages
     release v=13.5,o=Debian,a=stable,n=trixie,l=Debian,c=main,b=amd64
     origin deb.debian.org
```
...
$ apt-cache madison tree
      tree |    2.2.1-1 | http://deb.debian.org/debian trixie/main amd64 Packages
```
...
apt/stable,now 3.0.3 amd64 [installed,automatic]
apt-utils/stable,now 3.0.3 amd64 [installed]
bash/stable,now 5.2.37-2+b7 amd64 [installed]
binutils/stable,now 2.44-3 amd64 [installed,automatic]
build-essential/stable,now 12.12 amd64 [installed]
coreutils/stable,now 9.7-3 amd64 [installed,automatic]
curl/now 8.14.1-2+deb13u2 amd64 [installed,upgradable to: 8.14.1-2+deb13u3]
dash/stable,now 0.5.12-12 amd64 [installed,automatic]
```
...
Get:1 http://deb.debian.org/debian trixie/main amd64 tree amd64 2.2.1-1 [57.9 kB]
...
ii  binutils       2.44-3       amd64        GNU binary utilities
```
...
> nginx を削除するのは第19章の冒頭（ソースビルドの直前）です。
...
第19章では nginx のビルドを通じて、Linux システムの深い理解を養います。
...
次章では、`apt install` で動かした nginx の設定ファイル（`/etc/nginx/nginx.conf`）を実際に編集するために、Linux 標準のテキストエディタ3種（vim・nano・ed）を学びます。
